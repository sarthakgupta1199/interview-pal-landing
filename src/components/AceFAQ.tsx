...
const AceFAQ = () => {
  ...
  return <section id="faq" className="py-24 px-4 bg-muted/30">
          ...
          <div className="grid lg:grid-cols-2 gap-8 mb-12">
            <div>
              <Accordion type="single" collapsible className="space-y-4">
                {faqs.slice(0, 3).map((faq, index) => <AccordionItem key={index} value={`item-${index}`} className="bg-card rounded-lg border px-6">
            ...
            <div>
              <Accordion type="single" collapsible className="space-y-4">
                {faqs.slice(3).map((faq, index) => <AccordionItem key={index + 3} value={`item-${index + 3}`} className="bg-card rounded-lg border px-6">
                    <AccordionTrigger className="text-left text-base font-normal hover:no-underline py-6">
                      {faq.question}
                    </AccordionTrigger>
                    <AccordionContent className="text-muted-foreground pb-6">
